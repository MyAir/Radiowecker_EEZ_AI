...
static void alarm_edit_screen_load_handler(lv_event_t *e);
static void save_button_event_handler(lv_event_t *e);
static void cancel_button_event_handler(lv_event_t *e);
static void add_alarm_button_event_handler(lv_event_t *e);
void alarm_entry_click_event_handler(lv_event_t *e); // Needs to be global for AlarmManager
static void edit_button_event_handler(lv_event_t *e);
static void delete_button_event_handler(lv_event_t *e);
...
static void add_alarm_button_event_handler(lv_event_t *e) {
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        // The screen transition to alarm_edit_screen is handled by EEZ Studio
    }
}

// Event handler for loading the alarm edit screen
static void alarm_edit_screen_load_handler(lv_event_t *e) {
    lv_event_code_t code = lv_event_get_code(e);

    if (code == LV_EVENT_SCREEN_LOAD_START) {
        #if ALARM_DEBUG
        Serial.println("Alarm edit screen loading...");
        #endif
        AlarmManager* am = AlarmManager::getInstance();
        Alarm* alarm = am->getEditingAlarm(); // This will be nullptr for ADD mode

        if (am->getEditMode() == AlarmEditMode::ADD) {
            #if ALARM_DEBUG
            Serial.println("Mode: ADD. Setting default values.");
            #endif
            // Set default values for a new alarm
            lv_textarea_set_text(objects.alarm_title_textarea, "New Alarm");
            lv_roller_set_selected(objects.hour_roller, 0, LV_ANIM_OFF);
            ...
            lv_label_set_text(objects.alarm_date_label, "01.01.2025");
            lv_obj_clear_state(objects.alarm_repeat_switch, LV_STATE_CHECKED);
            
        } else if (alarm != nullptr) {
            #if ALARM_DEBUG
            Serial.printf("Mode: EDIT. Loading data for alarm ID %d.\n", alarm->id);
            #endif
            // Populate with existing alarm data
            ...
            if (alarm->repeat) lv_obj_add_state(objects.alarm_repeat_switch, LV_STATE_CHECKED); else lv_obj_clear_state(objects.alarm_repeat_switch, LV_STATE_CHECKED);
        }
    }
}
